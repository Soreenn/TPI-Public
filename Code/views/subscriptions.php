    ...
    <div class="overflow-x-auto">
        <?php foreach ($subjects as $subject) : ?>
            <?php foreach ($subscriptions as $subscription) : ?>
                <?php if ($subscription->Subject_id == $subject->id) : ?>
                    <?php if ((new User)->GetSpecificUserById($subject->User_id)[0]->blocked == 0) : ?>
                        <div class="mx-auto grid max-w-4xl grid-cols-12 gap-4 p-6">
                            ...
                            <?php if (count((new Post)->getMostRecentPostBySubjectId($subject->id)) > 0 && (new User())->GetSpecificUserById((new UserReplyPost())->getUserIdByPostId((new Post)->getMostRecentPostBySubjectId($subject->id)[0]->id)[0]->User_id)[0]->blocked == 0 && (new Post)->getMostRecentPostBySubjectId($subject->id)[0]->blocked == 0) : ?>
                            ...
                            <?php else : ?>
                                ...
                                <div class="col-span-12 rounded-lg bg-white sm:col-span-8 text-center">
                                    ...
                                    <?php if (substr(htmlentities($subject->media), -4) == ".png" || substr(htmlentities($subject->media), -4) == ".jpg" || substr(htmlentities($subject->media), -5) == ".jpeg" || substr(htmlentities($subject->media), -4) == ".gif") : ?>
                                        <img class="w-52 image mx-auto" src="<?= htmlentities($subject->media) ?>">
                                    <?php else : ?>
                                        <video class="w-96 mx-auto" controls>
                                            <source src="<?= htmlentities($subject->media) ?>" type="video/<?= substr($subject->media, -3) ?>" class="w-96 mx-auto">
                                            Your browser does not support the video tag.
                                        </video>
                                    <?php endif; ?>
                                </div>
